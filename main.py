...
with open(filename, 'w', newline="") as file:
    ...
    for row in csvreader :
        ...
        try:
            element = WebDriverWait(driver, 10).until(
                EC.presence_of_element_located(
                    (By.XPATH, "//span[text()='Love it again']"))
            )
            print('Devlog Cart Text Found')
        except:
            print('DEVLOG Cart Not Found Timed Out', row[0].strip())
            csvwriter.writerow([row[0].strip(), row[1].strip(), 'PASS', 'FAIL'])
            logout()
            continue

        try:
            driver.find_element(By.XPATH, "//h4[@class='z-2-text z-coast-base__article__name z-2-text-body-small-regular z-2-text-gray' and  text()='WS327 - Trainers - dark violet/glow']")
